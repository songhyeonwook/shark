...
from util_shw import BEVTransform, CURVEFit, draw_lane_img
...
class IMGParser:
    def __init__(self):
        ...
        self.upper_wlane = np.array([179,30,255])

        self.lower_ylane = np.array([12,65,30])
        self.upper_ylane = np.array([60,215,255])
        '''
        #lane rgb value
        self.lower_wlane = np.array([250,250,250])
        self.upper_wlane = np.array([255,255,255])

        self.lower_ylane = np.array([0,170,170])
        self.upper_ylane = np.array([100,255,255])
        

        #self.crop_pts = np.array([[[220,270],[420,270],[590,360],[50,360]]])
        self.crop_pts = np.array([[[200,290],[440,290],[620,360],[20,360]]])
    ...
    def binarize(self, img):
        ...
        # Update to directly use RGB color range
        img_rgb=img
        img_wlane = cv2.inRange(img_rgb, self.lower_wlane, self.upper_wlane)
        img_ylane = cv2.inRange(img_rgb, self.lower_ylane, self.upper_ylane)
...
if __name__ == '__main__':
    ...
    curve_learner = CURVEFit(order=3, lane_width=3 ,y_margin=1, x_range=30, min_pts=50)
    ...
    while not rospy.is_shutdown():
        ...
        if image_parser.img_bgr is not None:
            ...
            x_mid = x_pred
            y_mid = (y_pred_l + y_pred_r) / 2
           
            xyl, xyr, xym = bev_op.project_lane2img(x_pred, y_pred_l, y_pred_r, y_mid)
            ...
            img_lane_fit = draw_lane_img(img_lane, xyl[:, 0].astype(np.int32),
                                                ...
                                                xym[:, 0].astype(np.int32),
                                                xym[:, 1].astype(np.int32))
